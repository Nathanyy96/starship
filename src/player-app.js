(function () {
  ...
  function start() {
    ...
    function renderCharacterDetail(cardId) {
      ...
      var copies = state.collection[card.id] || 0; var progress = state.characterProgress[card.id] || { level: 1, affinity: 0, constellation: 0, constellationCore: 0 }; var cost = developmentCost(card, progress.level); var coreCost = constellationCost(progress.constellation || 0); var personalCores = Number(progress.constellationCore) || 0; var stats = characterStatsFor(card.id, progress) || {};
    ...
    function renderTrial() {
      ...
      byId("trial-team-list").innerHTML = owned.length ? owned.map(function (card) {
        ...
        return "<button class=\"trial-team-card " + (selected ? "selected" : "") + "\" data-trial-character=\"" + escapeHtml(card.id) + "\" type=\"button\"><span class=\"trial-team-art\" style=\"" + style + "\"><b>" + escapeHtml(card.element) + "</b><strong>" + escapeHtml(card.name) + "</strong></span><span class=\"trial-team-copy\"><strong>" + escapeHtml(card.name) + "</strong><small>" + escapeHtml(stats.role) + " · HP " + number(stats.maxHp) + "</small><small>攻 " + stats.attack + "／防 " + stats.defense + "／速 " + stats.speed + "</small><small>技能｜" + escapeHtml(stats.skillName || "—") + "</small></span><i>" + (selected ? "已編入" : "加入編隊") + "</i></button>";
