  ...
  function start() {
    ...
    function characterSkinMarkup(card, state, skins, activeId) {
      ...
      return "<div class=\"character-skin-list\">" + skins.map(function (skin) {
        var active = activeId === skin.id; var unlocked = characterSkinUnlocked(state, skin); var previewImage = skin.previewImage || card.image || card.backgroundImage || ""; var status = unlocked ? "已解鎖" : "尚未解鎖"; var buttonLabel = active ? "顯示原始立繪" : (unlocked ? "查看造型立繪" : "預覽造型");
    ...
    function renderCharacterDetail(cardId) {
      ...
      var skins = seasonSkinsFor(card.id); var activeSkin = skins.find(function (item) { return item.id === currentCharacterSkinId; }) || null; var skinActive = Boolean(activeSkin); var image = skinActive && activeSkin.previewImage ? activeSkin.previewImage : card.image || card.backgroundImage; var animation = characterAnimationFor(card.id);
