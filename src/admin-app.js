(function () {
  ...
  function start() {
    ...
    function render() {
      if (!state) return;
      setNumber("admin-star-sand", state.resources.starSand); setNumber("admin-star-marks", state.resources.starMarks); setNumber("admin-echo-powder", state.resources.echoPowder); setNumber("admin-character-exp", state.resources.characterExp);
    ...
    function save() {
      ...
      var body = {
        adminKey: key,
        name: player.name,
        resources: { starSand: Number(byId("admin-star-sand").value), starMarks: Number(byId("admin-star-marks").value), echoPowder: Number(byId("admin-echo-powder").value), characterExp: Number(byId("admin-character-exp").value) },
