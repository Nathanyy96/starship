...
def main():
    ...
    add_table(doc, ["項目", "目前設定", "狀態"], [
        ...
        ["劇情", "主線與支線 1.0–2.5，共 24 章，每章 3 幕", "開放"],
    ...
    add_text(doc, "版本標記為 2.0-2.5。伺服器在玩家建立帳號或登入時檢查 updateRewards.claimedVersions，只要該帳號尚未領取本版本，就加入 3,200 星砂並寫入領取時間。新帳號與舊帳號使用同一套邏輯，因此不會因為建立時間不同而漏領或重複領取。", after=4)
    add_bullets(doc, [
    ...
    add_table(doc, ["項目", "三星角色", "四星角色"], [
        ...
        ["升級倍率", "每級 2.2% 主要屬性、1.7% 防禦、0.7% 速度", "每級 2.8% 主要屬性、2.2% 防禦、0.9% 速度"],
        ["升級成本", "起始 60 角色經驗，每級增加 30", "起始 100 角色經驗，每級增加 55"],
    ...
    add_table(doc, ["委託", "區域與特色", "完成獎勵"], [
        ["潮汐書庫抄錄", "潮汐書庫；敵人數量較多，速度隊伍有優勢", "180 星砂、220 角色經驗、4 回響粉"],
        ["白帆岬補燈", "白帆岬；防禦較高，重裝或支援較穩定", "220 星砂、260 角色經驗、1 共鳴券"],
        ["鏡潮回收", "鏡潮島；敵方護盾與增益需要清除和控場", "260 星砂、300 角色經驗、1 星痕"],
