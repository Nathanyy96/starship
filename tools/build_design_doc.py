...
def main():
    ...
    add_text(doc, "本文件是目前遊戲程式的單一規則總覽，供製作、測試、部署與後續版本維護使用。現行玩家內容開放至 2.5；3.0–5.5 的內容已整理完成但維持鎖定。除抽卡、劇情、角色培養與戰鬥外，星海迷航和星伴培育也已納入同一套帳號保存規則。任何數值或開放狀態調整，都應同步檢查 src/data.js、玩家介面與測試。", size=10.8, after=10)
    ...
    add_table(doc, ["目標", "需求", "檢查結果"], [
        ...
        ["四名四星 1→90", "526,168 角色經驗", "目前版本列出的全部來源合計 541,200，餘 15,032"],
    ], widths=[1.55, 1.8, 3.25])
    add_bullets(doc, [
        "開放劇情 78 幕是一次性獎勵：每幕 +100 星砂、+650 角色經驗，重看不會重複領取。",
        ...
        "完整使用當期玩法上限可取得 28,860 星砂：18 次十連，並剩 60 星砂；抽卡重複角色帶來的經驗與每格 8% 隨機星砂未列入安全估算。",
    ])
    ...
    add_text(doc, "玩家進入劇情選單後可以切換主線與支線。章節清單只讓 1.0–2.5 可選；3.0–5.5 仍在同一份檔案中顯示為已建檔但鎖定。選定章節後，當幕內容與本章所有幕次會同時顯示，避免只看到劇情標題。", after=7)
    ...
    add_text(doc, "3.0–5.5 已完成內容設計並保存於資料層，目前不會被現行卡池或玩家劇情入口使用。1.0–2.5 的瑟蕾雅、獸靈之村與既有正史保留；3.0–3.5 改採『回聲之門』重製版，讓瑟蕾雅回到最初的家、面對艾妲的拒絕，再自然接入 4.0 的北境神話篇。每個大版本仍維持五個小版本，但新四星改為每個大版本 2–3 名；其餘已設計角色保留為劇情夥伴，依瑟蕾雅的旅程登場，不為了卡池硬切斷角色關係。3.2、4.2、5.3 另外保留一名三星，作為偶爾出現的低星角色。", after=7)
    add_table(doc, ["版本", "新角色", "劇情主題", "狀態"], [
        ["3.0–3.5", "霽羅 4★、蘿堤亞 4★、洛恩 4★；岑芽 3★", "回聲之門 → 門外先寫信；回到獸靈之村，尊重艾妲拒絕，再以北門接上神話篇", "3 名新四星／保留澄音、諾嵐、艾斯特於故事"],
    ...
    add_text(doc, "文件與程式目前以 2.0–2.5 為玩家版本基準；3.0–5.5 內容完成建檔但仍需正式版本公告、資產確認與回歸測試後才可開放。", size=10.5, bold=True, color="193B63", after=0)
