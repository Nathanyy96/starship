...
def main():
    ...
    add_text(doc, "本文件是目前遊戲程式的單一規則總覽，供製作、測試、部署與後續版本維護使用。現行玩家內容開放至 2.5；3.0–4.5 的內容已整理完成但維持鎖定。除抽卡、劇情、角色培養與戰鬥外，星海迷航和星伴培育也已納入同一套帳號保存規則。任何數值或開放狀態調整，都應同步檢查 src/data.js、玩家介面與測試。", size=10.8, after=10)
